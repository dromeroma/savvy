...
async def list_inventory(location_id: uuid.UUID | None = Query(None), db: AsyncSession = Depends(get_db), org_id: uuid.UUID = Depends(get_org_id)) -> Any:
...
async def adjust_stock(data: InventoryAdjustment, db: AsyncSession = Depends(get_db), org_id: uuid.UUID = Depends(get_org_id)) -> Any:
    return await InventoryService.record_movement(db, org_id, data, reference_type="adjustment")

@router.get("/movements", response_model=list[MovementResponse])
async def list_movements(product_id: uuid.UUID | None = Query(None), db: AsyncSession = Depends(get_db), org_id: uuid.UUID = Depends(get_org_id)) -> Any:
