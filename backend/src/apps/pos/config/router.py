...
async def list_taxes(db: AsyncSession = Depends(get_db), org_id: uuid.UUID = Depends(get_org_id)) -> Any:
...
async def create_tax(data: TaxCreate, db: AsyncSession = Depends(get_db), org_id: uuid.UUID = Depends(get_org_id)) -> Any:
    return await ConfigService.create_tax(db, org_id, data)

@router.get("/discounts", response_model=list[DiscountResponse])
async def list_discounts(db: AsyncSession = Depends(get_db), org_id: uuid.UUID = Depends(get_org_id)) -> Any:
...
async def create_discount(data: DiscountCreate, db: AsyncSession = Depends(get_db), org_id: uuid.UUID = Depends(get_org_id)) -> Any:
