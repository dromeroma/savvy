...
router = APIRouter(
    prefix="/fees",
    tags=["Condo Fees"],
    dependencies=[Depends(require_permission("condo", "fees.write", "reports.view"))],
)
_WRITE = [Depends(require_permission("condo", "fees.write"))]
...
@router.post("/generate", response_model=list[FeeResponse], status_code=status.HTTP_201_CREATED, dependencies=_WRITE)
...
@router.post("/{fee_id}/pay", response_model=FeeResponse, dependencies=_WRITE)
