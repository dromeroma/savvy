...
router = APIRouter(
    prefix="/maintenance",
    tags=["Condo Maintenance"],
    dependencies=[Depends(require_permission("condo", "maintenance.manage", "reports.view"))],
)
_WRITE = [Depends(require_permission("condo", "maintenance.manage"))]
...
@router.post("", response_model=MaintenanceResponse, status_code=status.HTTP_201_CREATED, dependencies=_WRITE)
...
@router.patch("/{req_id}", response_model=MaintenanceResponse, dependencies=_WRITE)
