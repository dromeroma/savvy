...
export class MoreAppsComponent implements OnInit {
  ...
  private loadData(): void {
    ...
    this.error.set('');
    // Active apps to exclude from the catalog (this page is for non-active apps only)
    this.appsService.getMyApps().subscribe({
      next: (apps) => this.loadCatalog(apps),
  ...
  private loadCatalog(myApps: MyApp[]): void {
    this.appsService.getCatalog().subscribe({
      next: (allApps) => {
        ...
        this.catalog.set(allApps.filter(a => !myCodes.has(a.code)));
